...
int main(int argc, char const *argv[])
{
    ...
    httpServer->socket_fd = socket_fd;

    // 创建线程池
    pthread_t pids[THREAD_NUM];
    for (int i=0;i<THREAD_NUM;i++)
    {
        int ret = pthread_create(&pids[i],NULL,worker,httpServer);
        if (ret!=0)
        {
...
void* worker(void* argv)
{
    HttpServer* httpServer = (HttpServer*)argv;
    ...
    event.data.fd = httpServer->socket_fd;

    if(epoll_ctl(epoll_fd, EPOLL_CTL_ADD, httpServer->socket_fd, &event) < 0)
    {
    ...
    while(1)
    {
        ...
        for(int i=0; i<eventCnt; i++)
        {
            ...
            if (eventList[i].data.fd == httpServer->socket_fd) {
                int ret = acceptConn(httpServer->socket_fd,epoll_fd);
                if (ret>0) {
